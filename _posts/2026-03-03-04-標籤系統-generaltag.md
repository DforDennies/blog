...
<!-- 📸 建議截圖：GeneralTag 三層分類體系圖——Root → Level 1 → Level 2 的階層結構，以健康類展開 22 子類為例（無需脫敏） -->
...
<!-- 📸 建議截圖：HMCN 模型架構圖——Root → L1 → L2 階層結構 + recursive regularization 示意（無需脫敏，為模型架構） -->
...
<!-- 📸 建議截圖：錯誤標籤案例截圖——展示 3-4 個語意邊界誤判的實際貼文和錯誤標籤（隱藏 KOL 帳號名稱，保留標籤和貼文摘要） -->
...
<!-- 📸 建議截圖：HMCN → LLM 技術世代對比圖——傳統模型（F-score 0.903）vs LLM 方案在語意邊界處理上的差異（無需脫敏） -->
