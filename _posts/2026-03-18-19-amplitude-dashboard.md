...
<!-- 📸 建議截圖：pipeline.py 的 CLI 執行過程——ingest → compute → export 的 terminal 輸出，含處理速度和檔案數（無需脫敏） -->
...
<!-- 📸 建議截圖：Dashboard v1 的 HTML 頁面截圖——七個 Tab 的概覽畫面，展示 Chart.js 圖表和 KPI 卡片（遮蔽客戶名稱） -->
...
<!-- 📸 建議截圖：Next.js v3 的 Route 結構和部署狀態——Vercel Dashboard 上的部署紀錄或 route 列表（無需脫敏） -->
...
<!-- 📸 建議截圖：Neon 512MB 用量儀表——顯示資料庫容量使用比例和各表佔用空間（無需脫敏） -->
