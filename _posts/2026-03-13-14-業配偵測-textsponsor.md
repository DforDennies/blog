...
<!-- 📸 建議截圖：TextSponsor 的六語言上線時間軸——從 zh 單語到六語言的擴展里程碑（無需脫敏） -->
...
<!-- 📸 建議截圖：TextSponsor 在資料管線中的位置——TextLangID → TextSponsor → TextKnowledge → ALS 的完整管線圖（無需脫敏） -->
...
<!-- 📸 建議截圖：日文 EDA 的業配標記詞頻分佈——「PR」「タイアップ」等標記的出現頻率（無需脫敏） -->
...
<!-- 📸 建議截圖：KOL Radar 產品端的業配歷史頁面——KOL Detail 頁的業配品牌列表和時間軸（脫敏 KOL 名稱） -->
