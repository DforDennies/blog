...
<!-- 📸 建議截圖：lex/vec/hyde 三條線架構圖——Query → 三路擴展 → 分別對應 BM25 和向量搜尋的完整流程（無需脫敏） -->
...
<!-- 📸 建議截圖：五步 Pipeline 流程圖——GPT 生成 → 品質驗證 → 格式轉換 → LoRA 訓練 → 推理部署的完整步驟（無需脫敏） -->
...
<!-- 📸 建議截圖：資料量 vs 品質曲線圖——200/500/1000/2000 筆資料量下的模型輸出品質變化和 sweet spot 標示（無需脫敏，為效能數據） -->
