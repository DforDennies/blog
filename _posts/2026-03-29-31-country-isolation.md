...
<!-- 📸 建議截圖：國家隔離架構總覽圖——ES/DataCenter/Crawler 三層隔離方式 + tw/global 雙 index 的整體設計（無需脫敏，為架構設計） -->
...
<!-- 📸 建議截圖：ES index 拆分策略圖——舊 akashic-kol-v1 → 新 akashic-kol-tw + akashic-kol-global 的 Alias 切換流程（隱藏具體 ES cluster URL） -->
...
<!-- 📸 建議截圖：雙寫入流程圖——check alias → 判斷新舊 index → 同時寫入舊+新 index 的決策邏輯（無需脫敏，為技術流程） -->
...
<!-- 📸 建議截圖：Crawler 排程策略對比圖——時間分配 vs Queue 細分 vs N 小時啟動三種策略的優劣比較（無需脫敏） -->
