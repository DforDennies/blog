...
<!-- 📸 建議截圖：ArgoCD 管理架構圖——CICD cluster 為中心，連線到 15+ 遠端 GKE cluster 的拓撲圖（隱藏 cluster URL 和 credential，保留拓撲結構） -->
...
<!-- 📸 建議截圖：四種 Application 管理模式對比圖——模式 A/B/C/D 的管理邊界和適用場景（無需脫敏，為架構設計） -->
...
<!-- 📸 建議截圖：ArgoCD 升級問題清單——Redis RDB 不相容 / secrets.yaml.dec 空檔 / Resource Tracking 模式變更的三個問題和解法（無需脫敏） -->
...
<!-- 📸 建議截圖：GitOps 部署流程圖——MR 提交 → Platform Review → Merge → ArgoCD Auto Sync → 遠端 Cluster 部署（隱藏 repo URL） -->
...
如果是全新的團隊，額外需要在 `credentialTemplates` 加入 GitLab Deploy Token。目前已有 7 組 credential template，涵蓋 ikala-infra、kolradar、cdp、ikala-ai、ai-architect、nexus。
